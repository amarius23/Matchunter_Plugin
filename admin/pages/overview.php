...
$data = generate_shortcodes($endpoints);
	...
	?>
	<div class = "row"> 
		<div class="col-6" style=" float:right; margin-right:5%; color:pink;">
			<h2 style="color:red;margin-top: 0;">Shortcode</h2>
			<?php
			if(empty($data)) echo '0 shortcodes were founded';
			else 
			foreach($data as $shortcode){
				echo '<p style="color:pink;margin-bottom:12%;margin-top:5%;">'.$shortcode.'</p>';
			}
			?>
		</div>
		<div class="col-6" style="color:blue; margin-left:10%; margin-right: 20%">
			<h2 style="color:blue;">Description</h2>
			<?php 
				if(empty($descriptions)) echo'Nothing to describe';
				else 
					foreach ($descriptions as $description) {
					echo '<p style="color:green;margin:4% 0; max-width:40%;">'.$description.'</p>';
				}
			?>
		</div>
	</div>
